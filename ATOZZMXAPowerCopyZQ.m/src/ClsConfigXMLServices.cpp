	...
	CATUnicodeString &ostrFeatVersion)
{
	...
	spTemplateNode->GetElementsByTagName("PCFeature", spChildNodeList);
	if (NULL_var == spChildNodeList) return E_FAIL;
	...
	for (int i = 0; i < iLength; i++)
	{
		...
		rc = S_OK;
		spChildElement->GetAttribute("PLM_ExternalID", ostrFeatID);
		spChildElement->GetAttribute("FeatVersion", ostrFeatVersion);
		break;
	}
